...
import type { KeyPair } from "https://raw.githubusercontent.com/nats-io/jwt.js/main/src/mod.ts";
import {
  Account,
  decode,
  fromSeed,
...
export interface Nsc {
  env(): Promise<Std>;
  addOperator(name?: string): Promise<[string, Std]>;
...
export const nsc: Nsc = {
  ...
  async addOperator(name?: string): Promise<[string, Std]> {
    name = name || nuid.next();
